...
import { parsePaymentSignatureHeader, sumVaultMintIncreaseBaseUnits, txIncludesSigner } from "./stakingVerify";
...
async function fetchSolanaTransaction(signature: string) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (json?.error) throw new Error(`RPC getTransaction error: ${JSON.stringify(json.error)}`);
      return json?.result;
    } catch (error: any) {
      lastErr = error;
  ...
  throw new Error(`RPC_UNAVAILABLE:${lastErr?.message || "unknown"}`);
}
...
router.route({
  path: "/api/stakes/list",
  method: "GET",
  handler: httpAction(async (ctx, req) => {
    const agentHandle = await getAuthenticatedAgent(ctx, req);
    if (!agentHandle) return jsonResponse({ ok: false, error: "Unauthorized" }, 401);

    const result = await ctx.runQuery(api.skillApi.listStakesByAgent, { agentHandle });
    return jsonResponse({ ok: true, ...result });
  }),
});

router.route({
  path: "/api/stakes/challenge",
  method: "POST",
  handler: httpAction(async (ctx, req) => {
    const agentHandle = await getAuthenticatedAgent(ctx, req);
    if (!agentHandle) return jsonResponse({ ok: false, error: "Unauthorized" }, 401);
    ...
    try {
      const result = await ctx.runMutation(api.skillApi.createStakeChallenge, {
        agentHandle,
        goalExternalId: String(body.goalExternalId ?? ""),
        stakingWallet: String(body.stakingWallet ?? ""),
        stakeAmount: Number(body.stakeAmount ?? 0),
        tokenSymbol: body.tokenSymbol ? String(body.tokenSymbol) : undefined,
        network: body.network ? String(body.network) : undefined,
      });
      return jsonResponse({ ok: true, ...result });
    } catch (error: any) {
      return jsonResponse({ ok: false, error: error?.message || "Stake challenge failed" }, 400);
    }
  }),
});

router.route({
  path: "/api/stakes/lock",
  method: "POST",
  handler: httpAction(async (ctx, req) => {
    const agentHandle = await getAuthenticatedAgent(ctx, req);
    if (!agentHandle) return jsonResponse({ ok: false, error: "Unauthorized" }, 401);
    ...
    const paymentSigRaw = getHeader(req, "PAYMENT-SIGNATURE");

    // First call: issue x402 challenge as HTTP 402
    if (!paymentSigRaw) {
      try {
        const challenge = await ctx.runMutation(api.skillApi.createStakeChallenge, {
          agentHandle,
          goalExternalId: String(body.goalExternalId ?? ""),
          stakingWallet: String(body.stakingWallet ?? ""),
          stakeAmount: Number(body.stakeAmount ?? 0),
        ...
        const paymentRequired = {
          ...
          scheme: "exact",
          network: "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp",
          payTo: "FBYd2KDzWuPUjUSbRb9koYcCTA3m5xdxaF18rPY888td",
          asset: {
            mint: "6CcfJRvjgDHfEsZnXDGg7mVSuvvtZqWukZ95fBqzpump",
            symbol: "$CLAWHAMMER",
            decimals: 6,
          },
          amountBaseUnits: challenge.requiredAmountBaseUnits,
          challengeRef: challenge.challengeRef,
          stakeId: challenge.stakeId,
          expiresAt: challenge.challengeExpiresAt,
        };

        return new Response(JSON.stringify({ ok: false, error: "Payment Required", paymentRequired }), {
          status: 402,
          headers: {
    ...
    // Second call: verify payment proof and finalize stake
    try {
      let parsed;
      try {
        parsed = parsePaymentSignatureHeader(paymentSigRaw);
      } catch (error: any) {
        return jsonResponse({ ok: false, error: error?.message || "Invalid PAYMENT-SIGNATURE" }, 400);
      }

      const { stakeId, txSignature, stakingWallet } = parsed;

      const stake: any = await ctx.runQuery(api.skillApi.getStakeByStakeId, { stakeId });
      if (!stake) return jsonResponse({ ok: false, error: "Stake not found" }, 404);
      if (stake.agentHandle !== agentHandle) return jsonResponse({ ok: false, error: "Stake ownership mismatch" }, 403);
      if (stake.status !== "pending_payment") return jsonResponse({ ok: false, error: "Stake is not awaiting payment" }, 400);
      if ((stake.challengeExpiresAt ?? 0) < Date.now()) return jsonResponse({ ok: false, error: "Stake challenge expired" }, 410);

      let tx: any;
      try {
        tx = await fetchSolanaTransaction(txSignature);
      } catch (error: any) {
        if (String(error?.message || "").startsWith("RPC_UNAVAILABLE:")) {
          return jsonResponse({ ok: false, error: "Temporary RPC issue, retry confirmation", retryable: true }, 503);
        }
        throw error;
      }
      if (!tx) return jsonResponse({ ok: false, error: "Transaction not found" }, 404);
      if (tx?.meta?.err) return jsonResponse({ ok: false, error: "Transaction failed on-chain" }, 400);
      if (!txIncludesSigner(tx, stakingWallet)) return jsonResponse({ ok: false, error: "Transaction signer does not match staking wallet" }, 400);

      const blockTimeMs = Number(tx?.blockTime ?? 0) * 1000;
      if (blockTimeMs > 0 && blockTimeMs < (stake.createdAt ?? 0)) {
        return jsonResponse({ ok: false, error: "Transaction predates challenge" }, 400);
      }

      const received = sumVaultMintIncreaseBaseUnits(
        tx,
        "FBYd2KDzWuPUjUSbRb9koYcCTA3m5xdxaF18rPY888td",
        "6CcfJRvjgDHfEsZnXDGg7mVSuvvtZqWukZ95fBqzpump"
      );

      const required = BigInt(String(stake.requiredAmountBaseUnits ?? "0"));
      if (received < required) {
        return jsonResponse({ ok: false, error: `Insufficient payment amount. Required=${required.toString()} received=${received.toString()}` }, 400);
      }

      await ctx.runMutation(api.skillApi.recordStakePaymentReceipt, {
        stakeId,
        txSignature,
        fromWallet: stakingWallet,
        toWallet: "FBYd2KDzWuPUjUSbRb9koYcCTA3m5xdxaF18rPY888td",
        mint: "6CcfJRvjgDHfEsZnXDGg7mVSuvvtZqWukZ95fBqzpump",
        amountBaseUnits: received.toString(),
      });

      const finalized = await ctx.runMutation(api.skillApi.finalizeStakePayment, {
        agentHandle,
        stakeId,
        txSignature,
        paymentSignatureHash: txSignature,
        tokenMint: "6CcfJRvjgDHfEsZnXDGg7mVSuvvtZqWukZ95fBqzpump",
      });

      return jsonResponse({ ...finalized, txSignature, receivedBaseUnits: received.toString() });
    } catch (error: any) {
      return jsonResponse({ ok: false, error: error?.message || "Stake verification failed" }, 400);
    }
  }),
});

router.route({
  path: "/api/stakes/confirm",
  method: "POST",
  handler: httpAction(async (ctx, req) => {
    const agentHandle = await getAuthenticatedAgent(ctx, req);
    if (!agentHandle) return jsonResponse({ ok: false, error: "Unauthorized" }, 401);
    ...
    try {
      const result = await ctx.runMutation(api.skillApi.confirmStakePayment, {
        agentHandle,
        stakeId: String(body.stakeId ?? ""),
        paymentSignatureHash: body.paymentSignatureHash ? String(body.paymentSignatureHash) : undefined,
        tokenMint: body.tokenMint ? String(body.tokenMint) : undefined,
      });
      return jsonResponse(result);
    } catch (error: any) {
      return jsonResponse({ ok: false, error: error?.message || "Stake confirm failed" }, 400);
    }
  }),
});
router.route({
  path: "/api/stakes/request-payout",
  method: "POST",
  handler: httpAction(async (ctx, req) => {
    const agentHandle = await getAuthenticatedAgent(ctx, req);
    if (!agentHandle) return jsonResponse({ ok: false, error: "Unauthorized" }, 401);
    ...
    try {
      const result = await ctx.runMutation(api.skillApi.requestStakePayout, {
        agentHandle,
        stakeId: String(body.stakeId ?? ""),
        evaluationId: String(body.evaluationId ?? "") as Id<"evaluations">,
      });
      return jsonResponse(result);
    } catch (error: any) {
      return jsonResponse({ ok: false, error: error?.message || "Payout request failed" }, 400);
    }
  }),
});
